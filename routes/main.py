...
#             # Only process if the file object has a filename attribute.
...
def confirm_purchase(event_id):
    ...
    try:
        ...
        flash(
            f"Tickets purchased successfully! You received {free_tickets} free ticket(s). Total Price: GHC{final_price:.2f}. Check your email for details.",
            "success")
...
# Updated E-Ticket PDF download route
@main.route("/ticket/<int:ticket_id>/download")
def download_ticket(ticket_id):
    ticket = Ticket.query.get_or_404(ticket_id)
    # Instead of login_required, secure the route by requiring a token query parameter
